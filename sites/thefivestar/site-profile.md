...
| 🟡 Med | Velocity Elementor at canonical staging slug (page 5107 since 2026-04-27); 2026-04-30 Template A REVISION applied (image-only hero with no overlay + 3-col WHEN/REGISTER/WHERE info bar + 20/20 padding cap on every body section); hero image populated (attachment 5143) + Alliance card image populated (attachment 5144); intro section H2 added ("Two days. One community."); 3 image slots remain (Section 2 community photo + FORCE card re-register + Section 8 Final CTA bg) | Pending Phase 3.11 production promotion ⏳ — also pending LLSS Template A revision so prod ships consistent event-page pattern |
| 🟡 Med | Events hub Elementor at canonical staging slug (page 5089 since 2026-04-27 PM via in-place swap; original WPBakery content backed up to `_elementor_inplace_swap_backup_*` meta) | Pending Phase 2.11 production promotion ⏳ |
| 🟡 Med | RE Pros Elementor at `/communities/real-estate-professionals/` (page 5109 since 2026-04-27); 301 redirect from `/memberships/real-estate-professionals/`; old WPBakery preserved at `/communities/real-estate-professionals-old/` | Pending Phase 4b.11 production promotion ⏳ |
| 🟡 Med | Memberships hub Elementor at canonical staging slug (page 5138; redesigned 2026-04-28 PM); 4 sections — "seven memberships" hero / 6-tile specialty grid (3-col CSS grid) / Alliance foundation strip below grid ("The Foundation" eyebrow + "underneath every specialty above") / footer-line; single navy band (hero only); PPEF copy fixed (membership org, not standards body); 7 logos uploaded | Pending Phase 4a-hub.11 production promotion ⏳ — minor polish may remain before promotion |
...
| 🟡 Med | Communities hub Elementor at canonical staging slug (page 5108 since 2026-04-28 PM via in-place swap; revised post-Jonathan-flow-correction to drop AMDC + MortgagePoint + membership logos; community-level copy + Memberships funnel CTA); 2026-04-29 PM Template C alignment applied (hero serif overrides dropped, single-navy-band rule via FSC bg navy to offwhite) | Pending Phase 4b-hub.11 production promotion ⏳ |
...
| 🟢 Low | Site footer "Membership Groups" widget links to `/memberships-old/` (post-Phase-4a-hub slug-swap drift) — should update to `/memberships/` on staging + prod | Cleanup item, not blocking |
